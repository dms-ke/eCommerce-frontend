...
export default function RegisterPage() {
  ...
  // 🔥 UPDATED: Added examples to the checklist labels
  const passwordCriteria = [
    { label: "At least 8 characters", met: password.length >= 8 },
    { label: "One uppercase letter (e.g., A, B, C)", met: /[A-Z]/.test(password) },
    { label: "One lowercase letter (e.g., a, b, c)", met: /[a-z]/.test(password) },
    { label: "One number (e.g., 1, 2, 3)", met: /\d/.test(password) },
    { label: "One special character (e.g., @, #, $, %, &, *)", met: /[^A-Za-z0-9]/.test(password) },
  ];
  // Check if ALL criteria are met to enable/disable the submit button
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // 🔥 NEW: Use the dynamic validation state instead of the old static regex
    if (!isPasswordValid) {
    ...
    try {
      const res = await fetch("http://localhost:3000/auth/register", {
        ...
        // Included fullName in case your backend is configured to save it!
    ...
    <main className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <button 
            type="submit"
            disabled={isLoading || !isPasswordValid} // 🔥 Disables button until all requirements are met
            className="w-full bg-blue-600 text-white font-bold rounded-xl px-4 py-3 mt-2 hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
          >
            {isLoading ? (
        ...
        <div className="mt-8">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4">
            <button 
              ...
              type="button"
              className="flex items-center justify-center gap-2 w-full border border-gray-300 rounded-xl px-4 py-2.5 hover:bg-gray-50 transition-colors font-medium text-gray-700"
            >
